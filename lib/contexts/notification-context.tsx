...
export const NotificationProvider: React.FC<NotificationProviderProps> = ({ children, userId }) => {
  ...
  // Convex mutations
  const markAsReadMutation = useMutation(api.mutations.notifications.markAsRead);
  const markAllAsReadMutation = useMutation(api.mutations.notifications.markAllAsRead);
  const deleteNotificationMutation = useMutation(api.mutations.notifications.deleteNotification);
  const clearAllMutation = useMutation(api.mutations.notifications.clearAllNotifications);
  const createNotificationMutation = useMutation(api.mutations.notifications.createNotification);

  // Handle notification actions
  const handleNotificationAction = React.useCallback((action: string, notificationId: Id<'notifications'>) => {
  ...
  const notifications = React.useMemo(() => {
    ...
    return convexNotifications.map((notification): Notification => ({
      id: notification._id,
      ...
      actions: notification.actions ? {
        primary: notification.actions.primary ? {
          ...
          action: () => handleNotificationAction(notification.actions!.primary!.action, notification._id),
        } : undefined,
        secondary: notification.actions.secondary ? {
          label: notification.actions.secondary.label,
          action: () => handleNotificationAction(notification.actions!.secondary!.action, notification._id),
        } : undefined,
      } : undefined,
  ...
  const addNotification = React.useCallback(async (notificationData: Omit<Notification, 'id' | 'timestamp'>) => {
    if (!userId) return;
